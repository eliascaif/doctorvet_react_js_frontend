...
const AddOwner = (props) => {
    ...
    useEffect(()=>{
        setData({...data, token:props.token, id_vet:props.idVet})
    },[]);
    ...
    const AddOwner = () => {
        if(!fetching && props.regions!==null){
            ...
            if(newData.nombre!=='' /*&& data.direccion!=='' && data.region!=='' && data.telefono!=='' && data.email!=='' && data.id_regional!=='' && !emailError*/){
                setFetching(true);
                AddNewOwner(newData, (response)=>{
        ...
        >
            <div className='new-owner-wrapper'>
                <div className='header-title'>
                    <div className='controls-new-owner'>
                        ...
                        <span style={{marginLeft:'1em'}}>
                            ...
                            <span className='owner-header-subtitle'>Ingresando nuevo propietario</span>
                        </span>
                    </div>
                        ...
                        <FormControl style={{width:'100%'}}>
                            ...
                            >
                                ...
                                props.regions.map(data=>{
                                    return <option key={data.id} value={data.id}>{data.nombre_region}</option>
                                })
                                }
                            </Select>
                        </FormControl>
                ...
                <Dialog
                    ...
                    open={fetchingError}
                    handleClose={()=>{setFetchingError(false);}}
                />
            </div>
        </Drawer>
    )
